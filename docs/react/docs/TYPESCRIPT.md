
# Typescript

- [Typescript redux tips](https://medium.com/@flushentitypack/typescript-redux-tips-that-make-developing-a-breeze-6202e586910)
-[React and Redux with TypeScript](https://levelup.gitconnected.com/react-and-redux-with-typescript-da0c37537a79)
- [Typescript in react](https://javascriptplayground.com/blog/2017/04/react-typescript/)
-[React and redux with typescript](https://medium.com/gitconnected/react-and-redux-with-typescript-da0c37537a79)
-[Static Type Checking (typescript and flow setup)](https://reactjs.org/docs/static-type-checking.html)
- [facebook alternative to typescript](https://flow.org)
- [React with typescript microsoft](https://github.com/Microsoft/TypeScript-React-Starter)

## Learn
- [Learn Typescript](https://www.reddit.com/r/typescript/comments/7s9bgp/essentials_reads_and_tools_to_know_for_learning/)